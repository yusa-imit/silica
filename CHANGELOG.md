...
- **Repo hygiene** (plan 001 item 2, part 2, batch 3): test scratch DBs in `storage/page.zig`
  (19), `storage/overflow.zig` (13), and `storage/fuzz.zig` (12) now write into a
  `std.testing.tmpDir` instead of the repo root — 44 occurrences, 3 files. 12 files
  (~1,441 occurrences) remain.
- **Repo hygiene** (plan 001 item 2, part 2, batch 4): test scratch DBs in
...
- **Repo hygiene** (plan 001 item 2, part 2, batch 7): test scratch DBs in `storage/btree.zig`
...
- **Repo hygiene** (plan 001 item 2, part 2, batch 9): test scratch DBs in `sql/catalog.zig`
  (152, uniform single-path pattern via the shared `TestCatalog` helper) now write into a
  `std.testing.tmpDir` instead of the repo root — 152 occurrences, 1 file. 2 files
  (~924 occurrences) remain, largest `sql/engine.zig` (819).
- **Repo hygiene** (plan 001 item 2, part 2, batch 10): test scratch DBs in `cli.zig` (105 — 98
