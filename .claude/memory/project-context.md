...
- **Tests**: 4483/4505 passed, 22 skipped, 0 failed (session end)
- **Later in session**: architect's plan landed (recorded in architecture.md) and flagged a real, independent MVCC bug — `ON CONFLICT DO UPDATE` wrote rows without a TupleHeader (engine.zig ~4777), silently bypassing snapshot isolation. Fixed via TDD (commit 95c29e3). Then implemented step 1/7 of the index-only-scan plan: `IndexInfo.covering_storage` catalog field (commit c43246c), also via TDD. 4 commits total this session: 9d7a358, 921ef3d, 95c29e3, c43246c.
- **Next priority**: Step 2/7 of the index-only-scan plan — `optimizer.zig` referenced-columns collection pass (see architecture.md for full detail; architect flagged this as a real prerequisite likely needing a small `Optimizer.optimize()` structural change for ancestor context, not a pure bolt-on).
