...
### SAVEPOINT rollback doesn't survive COMMIT (Session 495) — **OPEN, filed as issue #125, 7/8 steps done**
...
- **Fix decided (session 496)**: architect picked a **physical undo log** over sub-transaction IDs. 8-step phased plan (not persisted anywhere durable — reconstruct from commit messages/`.claude/memory` in the auto-memory system if needed): 1. core data structures ✅ (commit 275bbad) — `UndoRecord`, `TransactionContext.undo_log`, `Savepoint.undo_len` watermark, `Database.recordUndo()`, `Database.replayUndoTo()`. 2. wire plain INSERT to `recordUndo` ✅ (commit 8a437b1, session 497) — gated by a `via_on_conflict_update` flag so ON CONFLICT DO UPDATE rows (persisted at a different key) don't get a wrong "plain insert" undo entry; ON CONFLICT DO NOTHING already retracts via `continue` before reaching the call site. 3. wire DELETE to `recordUndo` ✅ (commit c26d340, session 498) — `executeDelete`'s row-collection loop now retains the raw pre-delete row bytes (new `DeleteEntry.raw_value` field, previously freed immediately after deserialization) and calls `recordUndo(table, key, raw_value, null)` right before each physical `tree.delete()`; every early-exit path in the collection loop had to be updated to free the now-longer-lived buffer. 4. wire UPDATE ✅ (commit 7394425, session 499) — needed both before_data and after_data; used the dupe-and-thread pattern since the row-collection loop has multiple early-exit paths. 5. wire `INSERT ... ON CONFLICT DO UPDATE` ✅ (commit 5c6db47, session 500) — simpler in-place `recordUndo` call, no duping needed since the mutation and the call site share a scope with no early exit between them. 6. wire MERGE ✅ (commit f952b49, session 501) — all 5 mutation sites in `executeMerge()` (WHEN MATCHED UPDATE/DELETE, WHEN NOT MATCHED INSERT, WHEN NOT MATCHED BY SOURCE UPDATE/DELETE); the two DELETE arms had no pre-delete row read at all and needed one added, the other three reused the step-5 in-place pattern. 7. cutover ✅ (commit 4b9d0dd, session 502) — `rollbackToSavepoint` (engine.zig ~1851-1883) now captures `saved_undo_len = txn.savepoints.items[idx].undo_len` before popping nested savepoints, and calls `self.replayUndoTo(txn, saved_undo_len)` in addition to the existing `resetCid`. **This was the actual bug fix — steps 1-6 only populated the log, this step is what consumes it.** The gap was a single missing function call; `replayUndoTo` itself had been fully implemented and unit-tested since step 1 (session 496). 8. separate whole-transaction-ROLLBACK bug fix — not started, and possibly not even a real bug (see below).
- **Current state**: `src/sql/engine.zig` test "SAVEPOINT: replace same-name savepoint" now asserts CORRECT post-commit behavior (`{1,2}`, row 3 does not reappear) and passes. Full suite 4536/4558 passed, 0 failed, 22 skipped.
- **Step 8/8 — needs verification, not yet started**: an older comment near engine.zig ~13512 claims whole-transaction `ROLLBACK` has an analogous bug, but `rollbackTransaction` (~1791-1807) uses `tm.abort(txn.xid)` (MVCC xid-level abort, making the whole xid globally invisible) rather than CID-based hiding — a fundamentally different and likely-already-correct mechanism, per a session-501 code investigation. **Before writing any implementation for step 8, first write a repro test** (`BEGIN; INSERT; INSERT; ROLLBACK; SELECT` — does the data actually come back or not?) to confirm whether this is a real bug or a stale comment. Don't assume more work is needed.
- **Status**: Not blocking CI. Tracked as GitHub issue #125 (bug label, priority 1 per CLAUDE.md issue protocol). 7/8 steps done; step 8 needs a repro test first to determine if it's even necessary.

### Host disk fills up from unbounded .zig-cache growth (Session 502) — **RECURRING, no automated fix**

Every disk-writing tool (build, git, subagent I/O) can fail with `ENOSPC` because this host runs cron for 5 sibling Zig projects (silica, sailor, zuda, zr, zoltraak) that each accumulate `.zig-cache` unboundedly — session 502 hit 120Mi free out of 228Gi, ~71G of which was `.zig-cache/o` (compiled object cache) across all 5 projects. Fix: `rm -rf .zig-cache` in the affected project only (safe — gitignored, regenerates on next `zig build`; this is CLAUDE.md's own documented "Clean" command). If you hit ENOSPC on a trivial command, run `df -h /` then `du -sh ~/codespace/*/.zig-cache` to find the culprit; only clean silica's own cache unless explicitly asked to touch siblings.

### Column-level UNIQUE constraint never enforced (Session 497) — **OPEN, filed as issue #126**

**Summary**: `CREATE TABLE t (id INTEGER UNIQUE)` parses `UNIQUE` into `ColumnInfo.flags.unique` but `Catalog.createTableFromAst` (catalog.zig) only auto-creates a secondary index for `flags.primary_key`, never `flags.unique` — so no index exists to enforce it, and duplicate values insert silently. Table-level `UNIQUE (col1, col2, ...)` constraints likely have the same gap (not verified). Workaround: explicit `CREATE UNIQUE INDEX` works correctly.
- **Found while**: writing a test for #125 step 2 — the test assumed inline `UNIQUE` would trigger `ON CONFLICT DO NOTHING`, and it silently didn't. Fixed the test to use `CREATE UNIQUE INDEX` instead.
- **Fix location**: the PK-only index-creation loop in `Catalog.createTableFromAst` needs an `or col.flags.unique` branch (with `IndexInfo.is_unique = true`), plus handling `TableConstraintInfo.unique` similarly.
- **Status**: not fixed. Issue #126, bug label, priority 1 per CLAUDE.md issue protocol.
