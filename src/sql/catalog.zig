...
pub const Catalog = struct {
    ...
    pub const DomainTypeInfo = struct {
        ...
        /// Constraint expression SQL text (e.g., "VALUE > 0").
        constraint: ?[]const u8,
        allocator: Allocator,
    ...
    fn makeDomainKey(self: *Catalog, name: []const u8) ![]const u8 {
        const prefix = "domain:";
        const key = try self.allocator.alloc(u8, prefix.len + name.len);
        @memcpy(key[0..prefix.len], prefix);
        @memcpy(key[prefix.len..], name);
    ...
    pub fn createDomain(
        ...
        // Serialize: [base_type: u8][constraint_present: u8][constraint_len: u16][constraint_text]
        const constraint_text = constraint orelse "";
        const total_size: usize = 1 + 1 + 2 + constraint_text.len;
        ...
        // Write base_type as u8
        data[offset] = @intFromEnum(base_type);
        offset += 1;

        // Write constraint_present flag
        data[offset] = if (constraint != null) 1 else 0;
        offset += 1;

        // Write constraint length and text
        std.mem.writeInt(u16, data[offset..][0..2], @intCast(constraint_text.len), .little);
        offset += 2;
        if (constraint_text.len > 0) {
            @memcpy(data[offset..][0..constraint_text.len], constraint_text);
        }
    ...
    pub fn getDomain(self: *Catalog, name: []const u8) !DomainTypeInfo {
        ...
        // Deserialize: [base_type: u8][constraint_present: u8][constraint_len: u16][constraint_text]
        if (value.len < 4) return CatalogError.InvalidSchemaData;

        var offset: usize = 0;
        const base_type: ast.DataType = @enumFromInt(value[offset]);
        offset += 1;

        const constraint_present = value[offset] != 0;
        offset += 1;
        ...
        const constraint = if (constraint_present and constraint_len > 0)
...
test "Catalog createDomain and getDomain" {
    const allocator = std.testing.allocator;
    const path = "test_catalog_domain.db";

    var tc = try TestCatalog.setup(allocator, path);
    defer tc.teardown(allocator);

    try tc.catalog.createDomain("positive_int", .type_integer, "VALUE > 0");

    const info = try tc.catalog.getDomain("positive_int");
    defer info.deinit();

    try std.testing.expectEqualStrings("positive_int", info.name);
...
test "Catalog createDomain duplicate error" {
    ...
    try tc.catalog.createDomain("positive_int", .type_integer, "VALUE > 0");
    try std.testing.expectError(CatalogError.TypeAlreadyExists, tc.catalog.createDomain("positive_int", .type_text, null));
}

test "Catalog dropDomain" {
    const allocator = std.testing.allocator;
    const path = "test_catalog_drop_domain.db";

    var tc = try TestCatalog.setup(allocator, path);
    defer tc.teardown(allocator);

    try tc.catalog.createDomain("positive_int", .type_integer, "VALUE > 0");
    try tc.catalog.dropDomain("positive_int", false);

    // Verify domain is gone
    try std.testing.expectError(CatalogError.TypeNotFound, tc.catalog.getDomain("positive_int"));
}

test "Catalog dropDomain IF EXISTS" {
    const allocator = std.testing.allocator;
    const path = "test_catalog_drop_domain_if_exists.db";

    var tc = try TestCatalog.setup(allocator, path);
    defer tc.teardown(allocator);

    // Drop non-existent domain with IF EXISTS should not error
    try tc.catalog.dropDomain("nonexistent", true);

    // Drop non-existent domain without IF EXISTS should error
...
test "Catalog domainExists" {
    ...
    try std.testing.expect(!try tc.catalog.domainExists("positive_int"));
    try tc.catalog.createDomain("positive_int", .type_integer, "VALUE > 0");
    try std.testing.expect(try tc.catalog.domainExists("positive_int"));
}
